...
* **The advantage of running a Bayesian mixed-effects model** 
* **Running Bayesian mixed-effects models using brms in R** 
...
# Frequestist approach: Mixed-effect model
...
mb1_data <- read_csv("https://raw.githubusercontent.com/manybabies/mb1-analysis-public/master/processed_data/03_data_trial_main.csv") %>% 
    ...
    mutate('Language background' = as.factor(
      ifelse(nae == T, 'North American English', 'non North American English')
      )) #rename nae for better labeling
...
mb1_data %>%
  mutate(age_group = factor(age_group, 
                            levels = c("3-6 mo", 
                                       "6-9 mo", 
                                       "9-12 mo", 
                                       "12-15 mo")), #fixing the order of age group
         trial_type = factor(trial_type, 
                             levels = c("IDS", "ADS"))
         ) #fixing the order of trial_type an d
...
4) North American English infants generally look longer to the the stimuli (North American English speech) than nin North American English infants.

5) The gap between IDS and ADS is larger for North American English infants in comparison to non North American English infants, suggesting that North American English infants may show stronger IDS preference than non North American English infants.

### Step 4: Analyzing the data

For simplicity, we focus on the random effects at the infant level (group) in this analysis. Based on the plot above, there are possibilities that infants differ from each other on (i) the magnitude of IDS preference (ii) rate of looking time decline over time. So in the following analyses, we would adopt the "Keep it maximal" approach (Barr et al., 2013) where we will include the maximal random effects structure justified by the design. As a first run, we first include an interaction term between IDS preference and rate of looking time decline as a random slope at the infant group level. However, as noted later, the interaction term cannot be modeled as a random effect because of singularity issue. So we remove the interaction term and only model the two main effects for the magnitude of IDS preference and looking time decline in the following model. 
...
Freq_model <- lmer(log_look_time ~ center_age_mo:trial_type + center_trial_num:trial_type + center_trial_order:trial_type + nae:trial_type + gender:trial_type + (1 + center_trial_num+trial_type|subid_unique), control=lmerControl(optimizer="bobyqa"), #using an optimizer that aligns with the version in MB1 and MB1B
                   data = mb1_data) 
...
```{r}
Bayes_model_default <- brm(looking_time ~ center_age_mo:trial_type + center_trial_num:trial_type + center_trial_order:trial_type + nae:trial_type + gender:trial_type + (1 + center_trial_num + trial_type | subid_unique),
                   family = lognormal(),
                   data = mb1_data,
                   warmup = 1000, 
                   iter = 4000, 
...
default_post <- mcmc_areas(posterior_default,
           pars = c("b_center_age_mo:trial_type",
                    "b_trial_type:center_trial_num",
...
Thus far, we did the Bayesian analysis using the default priors. But an important feature of Bayesian analysis is that we can incoporate our priors in our analysis. 
...
```{r}
Bayes_model_priors <- brm(looking_time ~ center_age_mo:trial_type + center_trial_num:trial_type + center_trial_order:trial_type + nae:trial_type + gender:trial_type + (1 + center_trial_num +trial_type |subid_unique),
                   family = lognormal(),
                   prior = priors,
                   data = mb1_data,
                   warmup = 1000, 
...
prior_post <- mcmc_areas(posterior_priors,
           pars = c("b_center_age_mo:trial_type",
                    "b_trial_type:center_trial_num",
                    "b_trial_type:center_trial_order",
                    "b_trial_type:naeTRUE"), #you can select which parameter(s) you want to plot
...
cowplot::plot_grid(default_post, prior_post, labels = c("Default", "With Prior"))

mb1_data_hpp <- length(unique(mb1_data$subid_unique))
```
...
```{r}
pp_check(Bayes_model_default)
```
Will the posterior predictive check be better after we have incorperated the priors?
```{r}
pp_check(Bayes_model_priors)
```

It doesn't look like it helps. So it may be a misspecification of the likelihood distribution. Let us look at the distribution of the looking time.
```{r}
hist(mb1_data$looking_time)
hist(mb1_data$log_look_time)
```
The log-transformed distribution is still not normal. This may be the reason why we were not able to have a good match between model fit and actual data. One new approach is to include a [mixture distribution for the looking time distribution](https://rdrr.io/cran/brms/man/mixture.html)